...
export default function NavBar({ toggleMobileMenu, isMobileMenuOpen }) {
  ...
  const DropDownMenuItem = ({heading, description}) => {
    return (
      <div className="flex justify-center items-center space-x-3 p-3 m-4 hover:bg-secondary-box hover:border-1 hover:border-white/20 hover:rounded-lg hover:border-dashed cursor-pointer">
        <div className="flex justify-center items-center h-[45px] w-[45px] border-1 border-white/20 rounded-lg ">
          ...
          <div>
            <h1>{heading}</h1>
            <p className="text-primary-text opacity-60">{description}</p>
          </div>
      </div>
    );
  };
      ...
      <div className="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 flex min-h-16 justify-between items-center rounded-full">
        ...
        <nav className="hidden items-center gap-6 lg:flex font-bold" aria-label="main-navigation">
          ...
          <div className="">
            ...
            <div className={`${isClicked ? '' : 'hidden'} absolute mt-3 grid grid-cols-2 h-75 w-150 border-1 border-white/20 rounded-lg`}>
